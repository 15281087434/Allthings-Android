...
public class CategoryTabStrip extends HorizontalScrollView {
	...
	private void calculateIndicatorLineRect(Rect rect) {
		ViewGroup currentTab = (ViewGroup)tabsContainer.getChildAt(currentPosition);
		TextView category_text = (TextView) currentTab.findViewById(R.id.category_text);
		...
		rect.set(((int) left) + getPaddingLeft()+20, currentTab.getTop() + getPaddingTop() + category_text.getTop() + category_text.getHeight()+10,
				((int) width) + getPaddingLeft()-20, currentTab.getTop() + getPaddingTop() + category_text.getTop() + category_text.getHeight()+20);
	}
	...
	public void draw(Canvas canvas) {
		super.draw(canvas);
		// 绘制高亮背景矩形红框
		calculateIndicatorLineRect(indicatorRect);
        ...
        while (i < tabsContainer.getChildCount()) {
            if (i < currentPosition - 1 || i > currentPosition + 1) {
                ...
                if (category_text != null) {
                    ...
                    textDrawable.setBounds(left, top+2, textDrawable.getIntrinsicWidth() + left, textDrawable.getIntrinsicHeight() + top);
