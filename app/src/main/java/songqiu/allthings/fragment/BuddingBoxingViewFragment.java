package songqiu.allthings.fragment;
...
public class BuddingBoxingViewFragment extends AbsBoxingViewFragment implements View.OnClickListener {
    public static final String TAG = "com.ccsuper.pudding.view.fragment.BuddingBoxingViewFragment";
    ...
    public static BuddingBoxingViewFragment newInstance() {
        return new BuddingBoxingViewFragment();
    }
    ...
    public void startLoading() {
        ...
        if (config.isNeedCamera() && config.isSingleImageMode() && !mIsCamera) {
            isOpenCamera();
            return;
        }
        loadMedias();
    ...
    private void isOpenCamera() {
        mIsCamera = true;
        startCamera(getActivity(), BuddingBoxingViewFragment.this, BoxingFileHelper.DEFAULT_SUB_DIR);
    }
    ...
    private void initViews(View view) {
        mEmptyTxt = view.findViewById(com.bilibili.boxing_impl.R.id.empty_txt);
        mRecycleView = view.findViewById(com.bilibili.boxing_impl.R.id.media_recycleview);
        mLoadingView = view.findViewById(com.bilibili.boxing_impl.R.id.loading);
        initRecycleView();

        boolean isMultiImageMode = BoxingManager.getInstance().getBoxingConfig().isMultiImageMode();
        View multiImageLayout = view.findViewById(com.bilibili.boxing_impl.R.id.multi_picker_layout);
        multiImageLayout.setVisibility(isMultiImageMode ? View.VISIBLE : View.GONE);
        if (isMultiImageMode) {
            mPreBtn = view.findViewById(com.bilibili.boxing_impl.R.id.choose_preview_btn);
            mPreBtn.setVisibility(View.GONE);
            mOkBtn = view.findViewById(com.bilibili.boxing_impl.R.id.choose_ok_btn);
    ...
    public void onCameraError() {
        ...
        dismissProgressDialog();
        //TODO 自改相机逻辑
        onFinish(new ArrayList<BaseMedia>());
    }
        ...
        if (id == com.bilibili.boxing_impl.R.id.choose_ok_btn) {
            ...
            if (!mIsPreview) {
                ...
                Boxing.get().withIntent(getActivity(), BoxingViewActivity.class, medias)
                        .start(this, IMAGE_PREVIEW_REQUEST_CODE, BoxingConfig.ViewMode.PREVIEW);
...
//                        .start(BuddingBoxingViewFragment.this, IMAGE_PREVIEW_REQUEST_CODE, BoxingConfig.ViewMode.EDIT);
