 ...
 *
 ********/
public class VideoDetailCommentAdapter extends RecyclerView.Adapter<VideoDetailCommentAdapter.ViewHolder> {
    ...
    public VideoDetailCommentAdapter(Context context, List<VideoDetailCommentBean> item) {
...
//        ((MessageViewholder)holder).isReadImg.setImageResource(R.mipmap.home_news);
        ...
        if(!StringUtil.isEmpty(videoDetailCommentBean.avatar)) {
            if(!videoDetailCommentBean.avatar.contains("http")) {
                videoDetailCommentBean.avatar = HttpServicePath.BasePicUrl+videoDetailCommentBean.avatar;
            }
        }
        ...
        holder.likeNumTv.setText(String.valueOf(videoDetailCommentBean.up_num));
            ...
            public void onClick(View v) {
                if(ClickUtil.onClick()) {
                    videoDetailCommentItemListener.toReport(videoDetailCommentBean.user_id,videoDetailCommentBean.commentid,
                            videoDetailCommentBean.article_id,2,position);
                }
            }
        });
    }
